...
(function(h) {
				...
				if (c === k || c === i)
					(j = l[i]),
						...
						j && j._bAttrSrc
							...
							: q ? (j._setter || (j._setter = N(j.mData)), j._setter(d, m)) : (d[i] = m);
				i++;
			};
		if (f)
			for (; f; ) {
	...
	s.registerPlural = u = function(a, b, c) {
		...
		s.register(b, function() {
			...
			return a === this
				? this
				: a instanceof s ? (a.length ? (h.isArray(a[0]) ? new s(a.context, a[0]) : a[0]) : k) : a;
		});
	};
	o('tables()', function(a) {
		var b;
	...
	n.defaults = {
		...
		oLanguage: {
			...
			sInfo: 'Hiển thị từ mục _START_ đến mục _END_ của _TOTAL_ mục',
			sInfoEmpty: 'Chưa có mục nào',

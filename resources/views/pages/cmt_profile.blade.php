                    ...
                    <div class="col-md-12">
                        @if(isset($data_comment))
                            @foreach($data_comment as $key_cmt => $value_cmt)
                                {{-- bình luận --}}
                                    <div class="media">
                                        <img class="mr-3 rounded-circle" alt="{{check_name($value_cmt['uid'])}}" src="{{ check_img(($value_cmt['uid'])) }}" />
                                        <div class="media-body">
                                            <div class="row">
                                                <div class="col-8 d-flex">
                                                    <a href="{{url('trang-ca-nhan/'.$value_cmt['uid'])}}" target="_blank"><h3>{{check_name($value_cmt['uid'])}}</h3></a> <small> {{date("Y-m-d H:i:s", $key_cmt)}}</small>
                                                </div>
                                                <div class="col-4">
                                            ...
                                            </div> <small>{!!$value_cmt['noi_dung']!!}</small>
                                            @include('pages.rep_profile')
                                            @if(isset($data_reply[$key_cmt]))
                                                @foreach($data_reply[$key_cmt] as $key_reply => $value_reply)
                                                    {{-- trả lời --}}
                                                        <div class="media mt-4">
                                                            <a class="pr-3" href="#"><img class="rounded-circle" alt="{{check_name($value_reply['uid'])}}" src="{{check_img($value_reply['uid'])}}" /></a>
                                                            <div class="media-body">
                                                                <div class="row">
                                                                    <div class="col-8 d-flex">
                                                                        <a href="{{url('trang-ca-nhan/'.$value_reply['uid'])}}" target="_blank"><h5>{{check_name($value_reply['uid'])}}</h5></a> <small> {{date("Y-m-d H:i:s", $key_reply)}}</small>
                                                                    </div>
                                                                    <div class="col-4">
                                                                ...
                                                                </div><small>{!!$value_reply['noi_dung']!!}</small>
                                                            </div>
                                                        </div>
                                                    {{-- end trả lời --}}
                                                @endforeach

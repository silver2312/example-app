...
$van_nang = VanNangModel::where('status',1)->get();
?>
<div class="table-responsive py-4">
    <table class="table table-flush" id="shop_vannang">
        ...
        <tbody>
            @foreach($van_nang as $key => $value)
                <tr>
                    ...
                    ?></td>
                    <td><a href="#!" class="btn btn-info"data-toggle="modal" data-target="#mua_van_nang_{{$value->id}}">Mua</a></td>
                </tr>
                    <!-- Modal -->
                    <div class="modal fade" id="mua_van_nang_{{$value->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form enctype='multipart/form-data' method="POST" action="{{ url('tu-luyen/cua-hang/van-nang/'.$value->id) }}">
                                        @csrf
                                        <div class="form-row">
                                        ...
                                        <button type="button" class="btn btn-secondary" onclick="close_cuahang_vn_{{$value->id}}()">Đóng</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <script >
                        function close_cuahang_vn_{{$value->id}}() {
                            $('#mua_van_nang_{{$value->id}}').modal('hide');
                        }
                    </script>
            @endforeach
        </tbody>
